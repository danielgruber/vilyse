	...
	public void export() {
		...
		if (userSelection == JFileChooser.APPROVE_OPTION) {
		    ...
		    try {
    		    ...
    		    double scale = c.project.getScale();
    		    System.out.println(scale);
    		    
    		    for(Picture p: this.pictures) {
    		    	double time = (p.getPicture() - c.project.getFirstPicture()) / c.project.getFps();
    		    	
    		    	if(p.getPoints().size() > 0) {
    		    	
    		    		writer.write(Double.toString(roundPrec(time, 2)));
    		    		...
    		    		for(PicturePointGroup pg : pointGroups) {
    		    			...
    		    			try {
    		    				...
    		    				if (rs.next()) {
    		    					...
    		    					writer.write(Double.toString(roundPrec(xCm, 2)));
    		    					writer.write(Double.toString(roundPrec(yCm, 2)));
    		    					writer.write(Double.toString(roundPrec(s, 2)));
    		    				} else {
    		    					writer.write("");
    		    					writer.write("");
    		    					writer.write("");
